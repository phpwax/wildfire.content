...
class WildfireContent extends WaxTreeModel {
  ...
  public function setup(){
    ...
    if(class_exists("WildfireUser")){
      $this->define("author", "ManyToManyField", array('target_model'=>"WildfireUser","eager_loading"=>true, "join_model_class"=>"WaxModelOrderedJoin", "join_order"=>"join_order", 'scaffold'=>true, 'group'=>'relationships', 'info_preview'=>1));
    }
  ...
  public function file_meta_set($fileid, $tag, $order=0, $title=''){
    ...
    foreach(CMSApplication::$handlers['media'] as $handler){
      $class = new $handler;
      $class->file_meta_set($this->primval, $fileid, $tag, $order, $title, $this->table, $this->primary_key);
    }
  }
  public function file_meta_get($fileid=false, $tag=false){
    ...
    foreach(CMSApplication::$handlers['media'] as $handler){
      $class = new $handler;
      $return = $class->file_meta_get($this->primval, $fileid, $tag, $this->table, $this->primary_key);
    }
    return $return;
